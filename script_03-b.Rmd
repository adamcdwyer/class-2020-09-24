...
sep_total <- inner_join(sep_old, sep_new, by = c("id", "title", "name", "department")) %>% 
  rename(u_grad_new = u_grad.y, u_grad_old = u_grad.x) %>% 
  mutate(u_grad_change = u_grad_old - u_grad_new)

increase <- sep_total %>% 
  arrange(u_grad_change) %>% 
  head(5)

gt(increase) %>% 
  tab_header(title = md("**Classes with Greatest Increase**"), 
                        subtitle = "From 9/1 to 9/22") %>% 
  cols_label(id = "Course ID",
             title = "Course Title",
             name = "Course Name", 
             department = "Department",
             u_grad_old = "9/1 Enrollment",
             u_grad_new = "9/22 Enrollment",
             u_grad_change = "Change")

decrease <- sep_total %>% 
  arrange(desc(u_grad_change)) %>% 
  head(5)

gt(decrease) %>% 
  tab_header(title = md("**Classes with Greatest Decrease**"), 
                        subtitle = "From 9/1 to 9/22") %>% 
  cols_label(id = "Course ID",
             title = "Course Title",
             name = "Course Name", 
...
change_join <- rbind(increase, decrease) %>% 
  ggplot(aes(x = fct_reorder(title, u_grad_change), y = u_grad_change, fill = department)) + 
  geom_col() +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45)) +
  scale_fill_brewer() +
  labs(title = "Change in Course Enrollment at Harvard 
       from September 1 to September 22",
       x = "Course Title",
       y = "Decrease in Enrollment")
...
# Outcome is sep_new and treatment is workload, since Dean Claybaugh is concerned about the level of work given during first few weeks
